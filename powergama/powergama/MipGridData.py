    ...
    def readFromFile(self,filename):
        with openfile(filename,'r') as csvfile:
            ...
            for row in datareader:
                ...
                self.loss.append(parseNum(row["loss"]))
                self.max_capacity.append(parseNum(row["max_capacity"]))
                self.fixed_cost.append(parseNum(row["fixed_cost"]))
                self.variable_cost.append(parseNum(row["variable_cost"]))
...
#                    self.capacity.append(row["capacity"])
    ...
    def writeToFile(self,filename):
        ...
        headers = ["from","to","reactance","capacity","loss","max_capacity","fixed_cost","variable_cost"]
        with openfile(filename,'w') as csvfile:
            ...
            for i in range(self.numBranches()):
                datarow = {"from":self.node_from[i],"to":self.node_to[i],\
                        ...
                        "loss":self.loss[i],"max_capacity":self.max_capacity[i],\
                        "fixed_cost":self.fixed_cost[i],"variable_cost":self.variable_cost[i]}
                datawriter.writerow(datarow)
        return
...
class _DcBranches(object):
    ...
    def writeToFile(self,filename):
        ...
        headers = ["from","to","capacity","loss","max_capacity","fixed_cost","variable_cost"]
        with openfile(filename,'w') as csvfile:
            datawriter = csv.DictWriter(csvfile, delimiter=',',\
                                quotechar='"', quoting=_QUOTINGTYPE,\
                                fieldnames=headers)
