...
class SipModel:
    ...
    def createScenarioTreeModel(self, num_scenarios, probabilities=None, stages=[1, 2]):
    ...
    def plotEnergyMix(
        ...
        if variable == "energy":
            ...
            for g in model.GEN:
                gen_output.append(
                    sum(model.generation[g, t, s].value for t in timeMaxMin)
                )
            title = "Energy mix"
        ...
        for typ in gentypes:
            ...
            for ar in model.AREA:
                prod = 0
                for g in model.GEN:
                    if (typ == model.genType[g]) & (
                        ar == model.nodeArea[model.genNode[g]]
                    ):
                        prod += gen_output[g]
                    else:
                        prod += 0
    ...
    def plotAreaPrice(
        ...
        if boxplot:
            ...
            for a in areas:
                ...
                factor[a] = {}
                areaprice[a] = [
                    self.computeAreaPrice(model, a, t, s) for t in timerange
                ]
            ...
            boxprops = dict(
                linestyle="--", linewidth=3, color="DarkOrange", facecolor="k"
            )
            flierprops = dict(
                marker="o", markerfacecolor="none", markersize=8, linestyle="none"
            )
            meanpointprops = dict(
                marker="D", markeredgecolor="red", markerfacecolor="red"
            )
            ...
            for typ in types:
                ...
                for c in model.LOAD:
                    welfare[typ][c] = (
                        sum(
                            [
                                self.computeAreaWelfare(model, c, t, s)[typ]
                                * model.samplefactor[t]
                                for t in model.TIME
                            ]
                        )
    ...
    def plotInvestments(self, filename, variable, unit="capacity"):
        ...
        if variable == "dcbranch":
            ...
            if not df_res["newCapacity"][df_res["newCapacity"] > 0].empty:
                ...
                ax2 = (
                    df_res[["cost_withOM", "congestion_rent"]][
                        df_res["newCapacity"] > 0
                    ]
                    .divide(10 ** 9)
                    .plot(
                        kind="bar",
        ...
        for b in model.BRANCH:
            for t in model.TIME:
                ...
                df_branch.loc[i, "utilization"] = (
                    model.branchFlow12[b, t, s].value
                    + model.branchFlow21[b, t, s].value
                ) / (
